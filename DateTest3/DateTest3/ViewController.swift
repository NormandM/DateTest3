...
class ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "Beginning of the Universe"
        infoDisplay.text = ""
...
// Creating resultMessageLabel
    ...
    func tableView(_ tableView: UITableView, titleForHeaderInSection section: Int) -> String? {
        ...
        if tableView == self.tableView {
            header = historicalData.sectionHeaderTable[0]
        }
        if tableView == self.tableView1 {
            header = historicalData.sectionHeaderTable1[0]
        }
        return header
    }
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        var count:Int?
        if tableView == self.tableView {
            count = historicalData.sectionHeaderTable.count
        }
        if tableView == self.tableView1 {
            count =  historicalData.sectionHeaderTable1.count
        }
        return count!
    }
 ...
 // Timing of the display of the dates for the quiz
    func runTimedCode() {
        ...
        if responseFromQuestionTimer.0{
            historicalData = responseFromQuestionTimer.1
            timer.invalidate()
            counter = 51
...
// this button an only be used once (there is only one extra event)
    ...
    @IBAction func showNextEventButton(_ sender: Any) {
        ...
        if n != i {swap(&historicalData.historicalEvent[i], &historicalData.historicalEvent[n])}
        if i != n + 1{swap(&historicalData.historicalEvent[i], &historicalData.historicalEvent[n + 1])}
        cellEvent = tableView1.cellForRow(at: [0, n]) as! CellForTableView
        cellEvent.eventCellLabel.text = historicalData.historicalEvent[n].0
        cellEvent = tableView1.cellForRow(at: [0, i]) as! CellForTableView
        cellEvent.eventCellLabel.text = historicalData.historicalEvent[i].0
        cellEvent = tableView1.cellForRow(at: [0, n + 1]) as! CellForTableView
        cellEvent.eventCellLabel.text = historicalData.historicalEvent[n + 1].0
